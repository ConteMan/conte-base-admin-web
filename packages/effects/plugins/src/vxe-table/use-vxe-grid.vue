...
const hasBuiltInToolbar = computed(() => {
  const toolbarConfig = gridOptions.value?.toolbarConfig;
...
const toolbarOptions = computed(() => {
  ...
  const isTreeGrid = Boolean(gridOptions.value?.treeConfig);
  ...
  const treeTools: VxeToolbarPropTypes.ToolConfig[] = isTreeGrid
    ? [
        {
          code: 'tree-expand-all',
          circle: true,
          icon: 'vxe-icon-square-plus',
          title: $t('common.expandAll'),
        },
        {
          code: 'tree-collapse-all',
          circle: true,
...
function onToolbarToolClick(event: VxeGridDefines.ToolbarToolClickEventParams) {
  ...
  if (event.code === 'tree-expand-all') {
    void gridRef.value?.setAllTreeExpand(true);
  }
  if (event.code === 'tree-collapse-all') {
    void gridRef.value?.setAllTreeExpand(false);
  }
  (
      ...
      :class="
        cn(
          ...
          'p-4',
          {
            ...
            'vben-vxe-grid--search-visible': !!formOptions && showSearchForm !== false,
          },
          gridClass,
        )
      "
